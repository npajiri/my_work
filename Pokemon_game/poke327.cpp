...
World world;
vector<Pokemon> choose_pokes;
vector<Pokemon> player_pokes;
extern int level;
pair_t all_dirs[8] = {
  { -1, -1 },
  { -1,  0 },
  ...
  */
  ...
  while (head) {
    ...
    if (x - 1 >= 0 && !m->map[y][x - 1]) {
      if ((rand() % 100) < 80) {
        m->map[y][x - 1] = /*(terrain_type_t) i*/ type;
        tail->next = (queue_node_t *) malloc(sizeof (*tail));
        tail = tail->next;
        tail->next = NULL;
        tail->x = x - 1;
        ...
        added_current = 1;
        m->map[y][x] = /*(terrain_type_t) i*/ type;
        tail->next = (queue_node_t *) malloc(sizeof (*tail));
        tail = tail->next;
        tail->next = NULL;
        tail->x = x;
    ...
    if (y - 1 >= 0 && !m->map[y - 1][x]) {
      if ((rand() % 100) < 20) {
        m->map[y - 1][x] = /*(terrain_type_t) i*/ type;
        tail->next = (queue_node_t *) malloc(sizeof (*tail));
        tail = tail->next;
        tail->next = NULL;
        tail->x = x;
...
void StartPc(){
  ...
  p2 = new Pokemon(1);
  p3 = new Pokemon(1);
// push them into player vectors
  choose_pokes.push_back(*p1);
  choose_pokes.push_back(*p2);        
  choose_pokes.push_back(*p3);
  p1->isDead = false;
  p2->isDead = false;
  p3->isDead = false;
  
  clear();
  printw("there %d pokemon in your back\n",choose_pokes.size());
  printw("The first poke info \n");
  ...
  printw("\n");
   printw("%s's moves: %s   moves: %s\n\n", p1->get_species(),
                   p1->get_move(0), p1->get_move(1));
  printw("The second poke info \n");
  printw("%s%s%s: HP:%d   ATK:%d   DEF:%d   SPATK:%d   SPDEF:%d   SPEED:%d %s",
  ...
  printw("\n");
   printw("%s's moves: %s   moves: %s\n\n", p2->get_species(),
                   p2->get_move(0), p2->get_move(1));
printw("The third poke info \n");
   printw("%s%s%s: HP:%d   ATK:%d   DEF:%d   SPATK:%d   SPDEF:%d   SPEED:%d %s",
  ...
  printw("\n");
   printw("%s's moves: %s   moves: %s\n\n", p3->get_species(),
                   p3->get_move(0), p3->get_move(1));
  printw("\n\n");



  printw("Please Choose your Pokemon\n");
  printw("Enter 1 to Choose %s%s%s \n", p1->is_shiny() ? "*" : "", p1->get_species(),p1->is_shiny() ? "*" : "");
  printw("Enter 2 to Choose %s%s%s \n",p2->is_shiny() ? "*" : "", p2->get_species(),p2->is_shiny() ? "*" : "");
  printw("Enter 3 to Choose %s%s%s \n",p3->is_shiny() ? "*" : "", p3->get_species(),p3->is_shiny() ? "*" : "");
  
  char ch = getch();
  bool out = false;
  while(out ==false){
  if(ch == '1'){
    printw("You have choose %s%s%s\n", p1->is_shiny() ? "*" : "", p1->get_species(),p1->is_shiny() ? "*" : "");
    player_pokes.push_back(*p1);
    out = true;
  }else if(ch == '2'){
    printw("You have choose %s%s%s\n",p2->is_shiny() ? "*" : "", p2->get_species(),p2->is_shiny() ? "*" : "");
    player_pokes.push_back(*p2);
   out = true;
  }else if(ch == '3'){
    printw("You have choose %s%s%s\n",p3->is_shiny() ? "*" : "", p3->get_species(),p3->is_shiny() ? "*" : "");
    player_pokes.push_back(*p3);
    out = true;
  }else{
    printw("wrong enter");
  }
  ...
  printw("You have %s%s%s in your Pokemon Bag\n", player_pokes[0].is_shiny() ? "*" : "", player_pokes[0].get_species(),player_pokes[0].is_shiny() ? "*" : "");
  getch();
  player_pokes[0].isActive = true;
...
void StartNpc(Npc *n){
  ...
  p1 = new Pokemon(level);
  p1->isDead = false;
  p1->cur_hp = p1->get_hp();
  n->Npc_pokes.push_back(*p1);
  int prob = rand()%10+1;
  clear();
  printw("level of this poke: %d\n", level);
   printw("NPC pokemon info: %s%s%s \n", p1->is_shiny() ? "*" : "", p1->get_species(),p1->is_shiny() ? "*": "");
   if(prob <= 6){
    Pokemon *p2;
    p2 = new Pokemon(level);
    p2->isDead = false;
    p2->cur_hp = p2->get_hp();
    n->Npc_pokes.push_back(*p2);
    printw("NPC pokemon info: %s%s%s \n", p2->is_shiny() ? "*" : "", p2->get_species(),p2->is_shiny() ? "*": "");
  }
  // poke3
 prob = rand()%10+1;
   if(prob <= 6){
    Pokemon *p3;
    p3 = new Pokemon(level);
    p3->isDead = false;
    n->Npc_pokes.push_back(*p3);
p3->cur_hp = p3->get_hp();
     printw("NPC pokemon info: %s%s%s \n", p3->is_shiny() ? "*" : "", p3->get_species(),p3->is_shiny() ? "*": "");
  ...
  prob = rand()%10+1;
   if(prob <= 6){
    Pokemon *p4;
    p4 = new Pokemon(level);
    p4->isDead = false;
    p4->cur_hp = p4->get_hp();
    n->Npc_pokes.push_back(*p4);

     printw("NPC pokemon info: %s%s%s \n", p4->is_shiny() ? "*" : "", p4->get_species(),p4->is_shiny() ? "*": "");
  

  prob = rand()%10+1;
   if(prob <= 6){
    Pokemon *p5;
    p5 = new Pokemon(level);
    p5->isDead = false;
    p5->cur_hp = p5->get_hp();
    n->Npc_pokes.push_back(*p5);

     printw("NPC pokemon info: %s%s%s \n", p5->is_shiny() ? "*" : "", p5->get_species(),p5->is_shiny() ? "*": "");
  
  
  prob = rand()%10+1;
   if(prob <= 6){
    Pokemon *p6;
    p6 = new Pokemon(level);
    p6->isDead = false;
    p6->cur_hp = p6->get_hp();
    n->Npc_pokes.push_back(*p6);

    printw("NPC pokemon info: %s%s%s \n", p6->is_shiny() ? "*" : "", p6->get_species(),p6->is_shiny() ? "*": "");
  }}}}
  // getch();
   
}
bool isGenerated_Pc = false;
int prob = rand()%10+1;
void game_loop()
{
  Character *c;
  ...
  while (!world.quit) {
    ...
    p = dynamic_cast<Pc *> (c);
    // adding pokemon or pokemons to each NPC 
if(n && !(n->isGenerated_Npc == true)){
    n->isGenerated_Npc =true;
      StartNpc(n);
    }
